...
const MENU_ITEMS = {
  admin: [
    ...
    { page: "reports", label: "Reports", href: "reports.html", icon: "materials" },
...
// Export table rows to CSV and printing helpers
function tableToCSV(table) {
  const rows = Array.from(table.querySelectorAll("tr"));
  return rows
    .map((row) =>
      Array.from(row.querySelectorAll("th,td"))
        .map((cell) => `"${String(cell.textContent || "").replaceAll('"', '""')}"`)
        .join(",")
    )
    .join("\n");
}
...
function exportTableToCSV(tableId, filename) {
  ...
  if (!table) return showToast("Table not found for export.", "error");
  const csv = tableToCSV(table);
  downloadFile(filename || `${tableId}.csv`, csv);
  showToast("Export started.");
}

async function reviewOuting(requestId, action) {
  if (!requestId || !action) return showToast("Invalid outing action.", "error");
  const comment = window.prompt("Optional comment for this decision (leave blank if none):", "");
  try {
    await api(`/outing/${requestId}`, { method: "PUT", body: JSON.stringify({ status: action, facultyComment: comment || "" }) });
    showToast(`Outing ${action} successfully.`);
...
// Lazy-load html2canvas and jsPDF for client-side PDF generation
...
function loadPdfLibraries() {
  ...
  return Promise.all(
    ...
    .catch((err) => {
      ...
      throw new Error("Unable to load PDF libraries.");
    });
}

async function saveSectionAsPdf(sectionId, filename = "export.pdf") {
  const section = document.getElementById(sectionId);
  if (!section) return showToast("Section not found for PDF.", "error");
  ...
  try {
    ...
    // jsPDF UMD exposes `window.jspdf` with `jsPDF` constructor
    const jsPdfGlobal = window.jspdf || window.jspdf || window.jspdf;
    const jsPDFClass = (jsPdfGlobal && (jsPdfGlobal.jsPDF || jsPdfGlobal.default?.jsPDF)) || window.jsPDF || null;
    ...
    // If content is taller than a single page, split into multiple pages
    const totalPages = Math.ceil(imgHeight / pageHeight) || 1;
    for (let p = 0; p < totalPages; p++) {
      const srcY = (p * pageHeight) / ratio;
      // create temporary canvas for the page slice
      const pageCanvas = document.createElement("canvas");
      pageCanvas.width = canvas.width;
      pageCanvas.height = Math.min(canvas.height - srcY, pageHeight / ratio);
      ...
      if (p > 0) pdf.addPage();
      pdf.addImage(pageData, "PNG", (pageWidth - imgWidth) / 2, 20, imgWidth, pageImgHeight);
    }

    pdf.save(filename);
    showToast("PDF saved.");
  } catch (error) {
    console.warn("PDF save failed, attempting server-side save", error);
    try {
      // create image data and send to server for reliable saving
      const canvas = await html2canvas(section, { scale: 2 });
      const imageData = canvas.toDataURL("image/png");
      const filenameSafe = `${slugify(filename.replace(/\.(pdf|png)$/i, "")) || "export"}-${Date.now()}.png`;
      const response = await api(`/reports/save`, {
        method: "POST",
        body: JSON.stringify({ filename: filenameSafe, imageData })
      });

      const downloadUrl = normalizeAssetUrl(response.url || `/uploads/reports/${response.filename}`);
      const a = document.createElement("a");
      a.href = downloadUrl;
      a.download = response.filename || filenameSafe;
      document.body.appendChild(a);
      a.click();
      a.remove();
      showToast("Saved to server and download started.");
    } catch (err) {
      console.error(err);
      // final fallback: open print dialog
      try {
        printTableSection(sectionId);
      } catch (err2) {
        console.error(err2);
        showToast(err.message || "Failed to save PDF or print.", "error");
      }
    }
  }
}

async function exportTimetableCSV(filename = "timetable.csv") {
  try {
    const data = await api(STATE.user.role === "student" ? `/timetable/${STATE.user.studentProfile?.id}` : "/faculty/timetable");
    const rows = data.timetable || [];
    if (!rows.length) return showToast("No timetable data to export.", "error");
    const csvLines = [["Day","Start","End","Course","Course Code","Faculty","Room"].join(",")]
      .concat(
        rows.map((r) =>
          [r.day_of_week, r.start_time, r.end_time, r.course_name, r.course_code, r.faculty_name || "", r.room_no || ""].map((v) => `"${String(v || "").replaceAll('"', '""')}"`).join(",")
        )
      )
      .join("\n");

    downloadFile(filename, csvLines);
    showToast("Timetable export started.");
  } catch (error) {
    showToast(error.message, "error");
  }
}
function printTableSection(sectionId) {
  const section = document.getElementById(sectionId);
  if (!section) return showToast("Section not found for print.", "error");
...
function createTableCard({ title, subtitle = "", headers, rows, emptyMessage = "No records found." }) {
  const tableId = `table-${slugify(title)}`;
  return `
    <section class="table-card" id="card-${slugify(title)}">
      <div class="table-head">
        ...
        <div class="table-actions">
          ${rows.length ? `<button class="button button-ghost button-small" type="button" onclick="exportTableToCSV('${tableId}','${slugify(title)}.csv')">Export CSV</button>` : ""}
          ${rows.length ? `<button class="button button-ghost button-small" type="button" onclick="saveSectionAsPdf('card-${slugify(title)}','${slugify(title)}.pdf')">Save PDF</button>` : ""}
          ${rows.length ? `<button class="button button-ghost button-small" type="button" onclick="printTableSection('card-${slugify(title)}')">Print</button>` : ""}
        </div>
      </div>
      ${
        rows.length
...
async function renderReportsPage() {
  ...
  renderLoading("Loading reports gallery...");

  try {
    const data = await api("/reports/list");
    const rows = (data.reports || []).map((file) => `
      <tr>
        <td><a href="${normalizeAssetUrl(file.url)}" target="_blank" rel="noreferrer">${escapeHtml(file.name)}</a></td>
        <td>${escapeHtml(file.name.split('.').pop() || '')}</td>
        <td>${escapeHtml(new Date(file.mtime).toLocaleString())}</td>
        <td><a class="button button-ghost button-small" href="${normalizeAssetUrl('/api/reports/download/' + encodeURIComponent(file.name))}">Download</a></td>
      </tr>
    `);

    pageContent.innerHTML = `
      ${createTableCard({ title: 'Reports Gallery', headers: ['File', 'Type', 'Modified', 'Actions'], rows, emptyMessage: 'No saved reports.' })}
    `;
  } catch (error) {
    pageContent.innerHTML = emptyState(error.message || 'Unable to load reports.');
  }
}
  ...
  if (STATE.user.role === "admin") {
    ...
    pageContent.innerHTML = `
      ...
      ${createTableCard({
        ...
        headers: ["Student", "Roll Number", "Purpose", "Dates", "Status", "Actions"],
        rows: pendingOutings.map(
          (request) => `
            <tr>
              ...
              <td>
                ${request.status === "pending" ? `<button class="button button-primary button-small" type="button" data-approve="${request.id}">Approve</button> <button class="button button-secondary button-small" type="button" data-reject="${request.id}">Reject</button>` : ""}
              </td>
            </tr>
          `
        ),
    ...
    document.querySelectorAll("[data-approve]").forEach((btn) => {
      btn.addEventListener("click", async () => {
        btn.disabled = true;
        await reviewOuting(btn.dataset.approve, "approved");
      });
    });
    document.querySelectorAll("[data-reject]").forEach((btn) => {
      btn.addEventListener("click", async () => {
        btn.disabled = true;
        await reviewOuting(btn.dataset.reject, "rejected");
      });
    });
    return;
  }
...
async function renderTimetablePage() {
  ...
  if (STATE.user.role !== "student") {
    ...
    pageContent.innerHTML = `
      ...
      <div class="table-head" style="margin-top:14px;">
        <div></div>
        <div class="table-actions">
          <button class="button button-ghost button-small" type="button" onclick="exportTimetableCSV()">Export CSV</button>
          <button class="button button-ghost button-small" type="button" onclick="saveSectionAsPdf('timetable-board','timetable.pdf')">Save PDF</button>
          <button class="button button-ghost button-small" type="button" onclick="printTableSection('timetable-board')">Print</button>
        </div>
      </div>
      <div id="timetable-board">${buildTimetableBoard(timetable)}</div>
    `;
