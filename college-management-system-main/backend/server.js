...
// optionally load report routes if server-side PDF support is installed
let reportRoutes = null;
try {
  reportRoutes = require("./routes/reportRoutes");
} catch (err) {
  console.warn("reportRoutes not available (pdfkit may be missing). Skipping report endpoints.", err?.message || err);
}
